...
2.  **Understand** — inspect types, completeness, distribution, and
    spread.
3.  **Table 1** — create the descriptive table for a report.
4.  **Compare groups** — answer one focused inferential question.
5.  **Crosstabs** — inspect categorical association and 2×2 measures.
6.  **History** — see which analyses were run during this session.
7.  **Help** — revisit the safe beginner workflow.
...
teaching datasets or upload your own data.
...
The **Table** panel is the publication-ready preview. Download buttons
provide DOCX, HTML, PDF, and RTF. PDF needs a working browser/webshot
setup; DOCX or HTML are the easiest choices for most users.
...
|----|----|
| Continuous, two independent groups | Welch t-test; Wilcoxon if marked skew is flagged |
| Continuous, 3+ independent groups | Welch ANOVA; Kruskal-Wallis if marked skew is flagged |
| Ordinal | Wilcoxon rank-sum or Kruskal-Wallis |
| Binary/nominal categorical | Chi-square when expected counts are adequate; Fisher exact when sparse |

You can select Student t-test, Welch t-test, Wilcoxon, ANOVA, Welch
ANOVA, Kruskal-Wallis, chi-square, or Fisher exact yourself. Use an
explicit choice only when it is justified by the study design or an
analysis plan.
