...
[![CRAN
version](https://www.r-pkg.org/badges/version/gtstats)](https://cran.r-project.org/package=gtstats)
[![CRAN downloads per
month](https://cranlogs.r-pkg.org/badges/last-month/gtstats)](https://cranlogs.r-pkg.org/downloads/total/last-month/gtstats)
...
customise_table()<br>save_output()
</p>
...
should be preserved exactly through `as_stats_table()`.
...
and PowerPoint. Use `customise_table()` to finish its appearance. Use
`to_gt()` only when an HTML-focused `gt` table is specifically required.
