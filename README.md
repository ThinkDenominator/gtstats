...
gtstats includes three labelled datasets, so examples work without installing
other packages:
...
prefer to learn by clicking through the workflow. It includes the three
teaching datasets, CSV/Excel upload, a working data dictionary, distribution
and variance checks, a layer-by-layer Summary table, focused group comparisons, crosstabs,
Word/HTML/PDF/RTF table downloads, an in-session history, and copyable or
downloadable R code for every analysis. Excel import uses the optional `rio`
package.
...
Until the package is on CRAN or has a DOI, use the package citation generated
by R:
